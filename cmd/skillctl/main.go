...
func main() {
	...
	// === end SPEC-0428 ===
	// === SPEC-0466 (with SPEC-0467 to SPEC-0471): Trust Freeze ===
	case "trust-freeze":
		os.Exit(runTrustFreeze(os.Args[2:], os.Stdout, os.Stderr))
	// === end SPEC-0466 ===
	// === end SPEC-0189 §14 ===
...
// printUsage prints the complete top-level verb surface.
//
// COMPLETE is the point, and it is enforced: cmd/docaudit reconciles the
// `switch os.Args[1]` dispatch above against the lines printed here, so a new
// `case` with no line in this function turns the docs gate red. Before that
// check existed, 29 of the 50 dispatched verbs were invisible to `skillctl
// help`, `pack` (step 2 of the advertised lifecycle) among them.
//
// The line format is load-bearing for that gate: a command line starts with
// EXACTLY two spaces, then the verb, and aliases follow it comma-separated
// (`help, --help, -h`). Continuation lines are indented further so they are not
// read as verbs. Group headers start at column zero.
func printUsage(w *os.File) {
	fmt.Fprintln(w, "Usage: skillctl <command> [args]")
	fmt.Fprintln(w, "")
	fmt.Fprintln(w, "The skill trust plane: pack, sign, publish, install, verify, audit, revoke.")
	fmt.Fprintln(w, "Run a command with --help for its flags. Ten answer only \"Unknown flag: --help\":")
	fmt.Fprintln(w, "scan, report, diff, seal, import, review, browse, consolidate, sync-usage, menubar.")
	fmt.Fprintln(w, "project, session and room want a subcommand first; verify-hook and enforce read a hook")
	fmt.Fprintln(w, "event on stdin and answer with a deny. The manual documents the flags of all of them.")
	fmt.Fprintln(w, "")
	fmt.Fprintln(w, "Getting started")
	fmt.Fprintln(w, "  help, --help, -h        Print this command list.")
	fmt.Fprintln(w, "  version, --version, -v  Print the build version.")
	fmt.Fprintln(w, "  doctor                  Readiness of this machine: version, home, skills dir, trust roots,")
	fmt.Fprintln(w, "                          peers, signing key, audit logs. Read-only. Exit 1 only when something")
	fmt.Fprintln(w, "                          is present and unusable; an absent config is a todo, not a failure.")
	fmt.Fprintln(w, "  login                   Browser device-pairing against ER1; saves a token skillctl then uses")
	fmt.Fprintln(w, "                          automatically. Flags: --no-browser, --timeout 5m, --base-url URL,")
	fmt.Fprintln(w, "                          --status, --logout.")
	fmt.Fprintln(w, "  token                   Fill the protected credential store the registry resolver reads,")
	fmt.Fprintln(w, "                          so no plaintext env var is needed. Subcommands: set | list | rm.")
	fmt.Fprintln(w, "")
	fmt.Fprintln(w, "Authoring and signing (SPEC-0188 Phase 1+2)")
	fmt.Fprintln(w, "  pack                    Build a deterministic .skb bundle from a skill directory.")
	...
	fmt.Fprintln(w, "  verify-sig              Verify a detached author signature locally, offline.")
	fmt.Fprintln(w, "")
	fmt.Fprintln(w, "Trust roots and federation (SPEC-0188 S7, SPEC-0359)")
	fmt.Fprintln(w, "  trust                   Manage ~/.claude/skill-trust-roots.yaml.")
	fmt.Fprintln(w, "                          Subcommands: list | add | add-author | fingerprint | remove.")
	fmt.Fprintln(w, "  peer                    Pin a federated peer registry and its key.")
	fmt.Fprintln(w, "                          Subcommands: add | ls | verify | rm.")
	fmt.Fprintln(w, "  cross-sign              Governance root vouching for a member reviewer key (SPEC-0359 D3).")
	fmt.Fprintln(w, "")
	fmt.Fprintln(w, "Admission and distribution (SPEC-0188 S9, SPEC-0194, SPEC-0225, SPEC-0246, SPEC-0272)")
	fmt.Fprintln(w, "  attest                  POST a signed governance attestation to the registry.")
	fmt.Fprintln(w, "  propose                 The ready-to-promote gate: is this skill fit for admission.")
	fmt.Fprintln(w, "  revoke                  Revoke an admitted bundle, and drive the kill-switch feed.")
	fmt.Fprintln(w, "                          Forms: revoke <digest> --reason <enum> | revoke feed [--status|--refresh].")
	...
	fmt.Fprintln(w, "  pull                    Run the 5-gate gauntlet against the registry; stage verified bundles only.")
	fmt.Fprintln(w, "  registry                Inspect a registry. Subcommands: ls [--latest] | show <name|sha256:…> |")
	fmt.Fprintln(w, "                          init | export.")
	fmt.Fprintln(w, "  room                    Share or unshare a published skill into a co-learning room.")
	fmt.Fprintln(w, "  runbook                 Publish an onboarding runbook into the THOH catalog (runbook publish).")
	fmt.Fprintln(w, "")
	fmt.Fprintln(w, "Install and verify (SPEC-0188 S8, SPEC-0276, SPEC-0406)")
	fmt.Fprintln(w, "  install                 Pull, verify, and install a signed skill bundle.")
	fmt.Fprintln(w, "  verify                  Re-run the trust-chain check on an installed skill or a .skb file.")
	fmt.Fprintln(w, "  export-bundle           Produce a sendable bundle plus its verification envelope.")
	fmt.Fprintln(w, "  export-verification-kit Write a portable, offline, trust-nothing verification kit for a bundle.")
	fmt.Fprintln(w, "  compliance              Render an offline compliance evidence pack (compliance report --framework).")
	fmt.Fprintln(w, "")
	fmt.Fprintln(w, "Enforcement gate (SPEC-0247, SPEC-0317)")
	...
	fmt.Fprintln(w, "  guard-path              Side-channel guard for Bash/Read/Edit/Write on a skill dir. Detection,")
	fmt.Fprintln(w, "                          not a seal: see `skillctl guard-path --explain`.")
	...
	fmt.Fprintln(w, "  session-baseline        Print the named offline state (online/degraded/offline/locked).")
	fmt.Fprintln(w, "                          Informational SessionStart context; gates nothing.")
	fmt.Fprintln(w, "")
	...
	fmt.Fprintln(w, "  trust-freeze            Capture, approve, verify and diff a signed host state (SPEC-0466), offline.")
	...
	fmt.Fprintln(w, "  sync                    Drain the device-signed audit_events outbox to the evidence ingest.")
	fmt.Fprintln(w, "                          A separate process; never invoked on the hook path.")
	...
	fmt.Fprintln(w, "  translog consistency    Check two signed tree heads are append-only consistent.")
	fmt.Fprintln(w, "  translog witness        Cross-witness STHs for a split view (equivocation).")
	fmt.Fprintln(w, "")
	fmt.Fprintln(w, "Agent identity, awareness and intent (SPEC-0195, SPEC-0277)")
	...
	fmt.Fprintln(w, "  awareness reset         Reset a session's admissions (two-step confirm).")
	fmt.Fprintln(w, "  intent declare          Declare a bundle's SPEC-0196 data-scope intent.")
	fmt.Fprintln(w, "  intent show             Show the declared intent for a skill or digest.")
	fmt.Fprintln(w, "")
	fmt.Fprintln(w, "Inventory and analysis (SPEC-0189 S0a, SPEC-0246 §4.5)")
	fmt.Fprintln(w, "  scan                    Scan skill directories and emit a JSON inventory.")
	fmt.Fprintln(w, "                          `scan --body <skill-dir>` is the separate behavioural bodyscan.")
	fmt.Fprintln(w, "  report                  Render a scan into an HTML or Markdown report.")
	fmt.Fprintln(w, "  diff                    Diff two scans (--output json|md|html|text).")
	...
	fmt.Fprintln(w, "  sync-usage              Sync local skill-usage counters to ER1.")
	fmt.Fprintln(w, "")
	fmt.Fprintln(w, "Workspace context (SPEC-0213, SPEC-0214)")
