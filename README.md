...
- Set `ER1_ENV=prod`
- Set `ER1_VERIFY_SSL=true`

If `ER1_ENV=prod` and TLS verification is disabled, uploads fail fast by design.

## Audio Import Modes

`import-audio` supports three modes:

- `--dry-run` (default): scan + status report only
- `--apply`: import files into MEMORY folders and update tracking DB
- `--upload`: import and upload imported files to ER1
