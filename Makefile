...
# Grant macOS privacy permissions. Screen Recording + Microphone are required.
# Accessibility and Input Monitoring are optional (only needed for advanced
# simulated-keyboard interaction flows). Opens System Settings panes one at a
# time and waits for user confirmation.
.PHONY: permissions
permissions:
	...
	@echo ""
	@echo "Optional advanced permissions:"
	@echo "  3. Accessibility     — advanced window/app interaction"
	@echo "  4. Input Monitoring  — simulated keystroke shortcuts"
	@echo ""
...
# Runs docs consistency check before release.
.PHONY: release release-patch release-minor release-major
release: check-docs release-patch

release-patch:
	@./scripts/release.sh patch

release-minor: check-docs
	@./scripts/release.sh minor

release-major: check-docs
	@./scripts/release.sh major

# --- Documentation ---

# Serve Jekyll docs locally (http://localhost:4000/m3c-tools/)
.PHONY: docs-serve
docs-serve:
	@echo "Serving docs at http://localhost:4000/m3c-tools/"
	@cd docs && bundle exec jekyll serve --livereload 2>/dev/null || \
		(echo "Install Jekyll: gem install bundler jekyll && cd docs && bundle install" && exit 1)

# Deploy docs to gh-pages branch
.PHONY: docs-deploy
docs-deploy: check-docs
	@echo "Deploying docs to gh-pages..."
	@DOCS_TMPDIR=$$(mktemp -d) && \
		cd docs && \
		if command -v bundle >/dev/null 2>&1 && [ -f Gemfile ]; then \
			bundle exec jekyll build -d "$$DOCS_TMPDIR" 2>/dev/null; \
		else \
			jekyll build -d "$$DOCS_TMPDIR" 2>/dev/null || \
			(echo "Jekyll not found. Deploying raw markdown..."; cp -r . "$$DOCS_TMPDIR"); \
		fi && \
		cd .. && \
		...
		touch .nojekyll && \
		git add -A && \
		git commit -m "docs: update gh-pages from $$(git log --format='%h' -1 master 2>/dev/null || echo 'HEAD')" && \
		git push origin gh-pages && \
		git checkout - && \
		git stash pop -q 2>/dev/null || true && \
		rm -rf "$$DOCS_TMPDIR" && \
		echo "Deployed to https://kamir.github.io/m3c-tools/"

# Check docs consistency with implementation
.PHONY: check-docs
check-docs:
...
help:
	...
	@echo "  docs-serve     Serve Jekyll docs locally"
	@echo "  docs-deploy    Deploy docs to gh-pages branch"
	@echo "  check-docs     Check docs consistency with implementation"
