...
func TestER1EnqueueFailure(t *testing.T) {
	...
	t.Setenv("ER1_MEMORY_PATH", filepath.Join(tmpDir, "MEMORY"))
	...
	// Verify entry fields
	if filepath.Base(entry.TranscriptPath) != "vid123_transcript.txt" {
		t.Errorf("Expected transcript filename vid123_transcript.txt, got %s", entry.TranscriptPath)
	}
	if filepath.Base(entry.AudioPath) != "vid123_audio.wav" {
		t.Errorf("Expected audio filename vid123_audio.wav, got %s", entry.AudioPath)
	}
	if filepath.Base(entry.ImagePath) != "vid123_thumbnail.jpg" {
		t.Errorf("Expected image filename vid123_thumbnail.jpg, got %s", entry.ImagePath)
	}
	...
	if entry.MemoryPath == "" {
		t.Error("Expected non-empty MemoryPath")
	}
