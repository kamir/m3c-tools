...
func StartBackgroundRetry(queuePath string, cfg *Config, interval time.Duration, maxRetries int) *BackgroundRetry {
	...
	uploadFn := func(entry QueueEntry) error {
		transcriptData, err := os.ReadFile(entry.TranscriptPath)
		if err != nil {
			return fmt.Errorf("read transcript %s: %w", entry.TranscriptPath, err)
		}
		audioData, audioName, err := retryAsset(entry.AudioPath)
		if err != nil {
			return err
		}
		imageData, imageName, err := retryAsset(entry.ImagePath)
		if err != nil {
			return err
		}
		payload := &UploadPayload{
			TranscriptData:     transcriptData,
			TranscriptFilename: filepath.Base(entry.TranscriptPath),
			AudioData:          audioData,
...
func retryAsset(path string) ([]byte, string, error) {
	if path == "" {
		return nil, "", nil
	}
