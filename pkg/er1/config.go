...
func (c *Config) Summary() string {
	...
	return fmt.Sprintf("ER1 -> %s key=%s ctx=%s timeout=%ds env=%s verify_ssl=%v",
		c.APIURL, masked, c.ContextID, c.UploadTimeout, c.Environment, c.VerifySSL)
}

// IsProd reports whether ER1 is running in production mode.
func (c *Config) IsProd() bool {
	return strings.EqualFold(strings.TrimSpace(c.Environment), "prod")
}

// ValidateTLSPolicy enforces stricter TLS settings in production.
func (c *Config) ValidateTLSPolicy() error {
	if c.IsProd() && !c.VerifySSL {
		return fmt.Errorf("ER1 TLS policy violation: ER1_ENV=prod requires ER1_VERIFY_SSL=true")
	}
	return nil
}
